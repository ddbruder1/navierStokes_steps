'''
CFD ME702 Step 2 // 1D Non-linear convection //

eq: u[i]@ n+1 = u[i] @ n -u[i](dt/dx)*(u[i] @n - u[i-1] @n)
in this eq:  transporting some initial profile "u" with velocity u^2
...
'or use another set of points'
nx = 51 #number of points in the grid
dx = 2.0/(nx-1) #distance between adjacent grid points (dx) 0.05 * 50 steps
dt = 0.01 #lenght of time step
